...
const DEFAULT_NAVBAR = {
  ...
  signInButton: 'Client Portal',
  signInHref: '/portal',
  getStartedButton: 'Get Quote',
  getStartedHref: '/quote',
} as const;
...
export default function Navigation(props: NavbarProps) {
  ...
  // ✅ FIXED: Navigation items with editableId for tracking
  const navigation = [
  {
    name: "Home",
    href: "/"
            ...
            <div className="ml-10 flex items-baseline space-x-8">
              {navigation.map(item => (
                <Link
                  key={item.name}
                  href={item.href}
                  data-editable-href={item.editableId}
                  data-original-href={item.href}
                  className={cn(
